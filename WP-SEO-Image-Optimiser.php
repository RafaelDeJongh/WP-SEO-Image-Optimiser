...
foreach($currentMediaLibrary->posts as $image){
	//Check if alt tag exists - if not copy it over from the title and sanetise both title and alt.
...
function seoio_title_to_alt_callback(){
	...
	$titleToAlt = get_option("seoio_title_to_alt");
	if($titleToAlt == 1){
		echo ' checked';
	}
	echo '/>';
...
function seoio_title_to_desc_callback(){
	...
	$titleToDesc = get_option("seoio_title_to_desc");
	if($titleToDesc == 1){
		echo ' checked';
	}
	echo '/>';
...
function seoio_title_to_caption_callback(){
	...
	$titleToCaption = get_option("seoio_title_to_caption");
	if($titleToCaption == 1){
		echo ' checked';
	}
	echo '/>';
